...
function selectLanguage(langCode) {
  setLang(langCode); // syncs vue-i18n locale + uiStore.lang + localStorage atomically
  isOpen.value = false;
}
</script>
...
<template>
  <div class="relative no-print z-50">
    ...
    <div
      v-if="isOpen"
      class="absolute top-full right-0 mt-2 w-36 bg-white rounded-xl shadow-xl border border-stone-100 overflow-hidden py-2 animate-in fade-in slide-in-from-top-2"
    >
      <button
