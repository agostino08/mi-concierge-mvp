...
// FUNCIÓN DE PDF CORREGIDA (No bloquea la app)
const downloadPDF = () => {
  const element = document.getElementById("itinerary-pdf-content");
  
  if (!element) return;

  // Verificamos que la librería del index.html esté lista
  if (!window.html2pdf) {
  ...
  const opt = {
    ...
    html2canvas: { 
      scale: 2, // Bajamos a 2 para mayor fluidez y evitar bloqueos
      useCORS: true, 
...
<template>
  <div class="space-y-8 animate-in fade-in duration-500">
    ...
    <div class="max-w-xs mx-auto pt-4 no-pdf">
      <button @click="downloadPDF" class="btn-dark w-full py-6 flex items-center justify-center gap-3">
        Descargar Guía PDF
      </button>
    </div>
  </div>
