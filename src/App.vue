  ...
  <div class="min-h-screen bg-gray-50 font-sans text-gray-900 pb-10">
    ...
    <main class="max-w-md mx-auto p-4">
      ...
      >
        ...
        <div class="space-y-6">
          <div>
            <label class="text-xs font-black text-gray-400 uppercase tracking-tighter">¿Con quién viajas?</label>
            <div class="flex flex-wrap gap-2 mt-2">
              <button
                v-for="opt in options.group"
                :key="opt"
                @click="formData.group = opt"
                :class="formData.group === opt ? 'bg-black text-white shadow-lg' : 'bg-white border text-gray-600'"
          ...
          <div class="grid grid-cols-1 gap-6">
            <div>
              <label class="text-xs font-black text-gray-400 uppercase">¿Cuántos días?</label>
              <select
            ...
            <div>
              <label class="text-xs font-black text-gray-400 uppercase tracking-tighter">¿Qué te apetece? (Varios)</label>
              <div class="flex flex-wrap gap-2 mt-2">
      ...
      >
        ...
        <p class="mt-10 text-2xl font-black leading-tight">{{ loadingPhrases[loadingStep] }}</p>
      ...
      >
        <div class="flex justify-between items-center mb-8">
          <button @click="itinerary = null" class="text-xs font-black uppercase tracking-widest text-gray-400">← Volver</button>
          <div class="flex gap-2">
            <button @click="shareWhatsApp" class="bg-green-500 text-white p-3 rounded-xl shadow-md">
            ...
            <button @click="downloadPDF" class="bg-black text-white p-3 rounded-xl shadow-md text-xs font-bold uppercase">PDF</button>
          </div>
        </div>

        <div id="itinerary-content">
          <div v-for="day in itinerary" :key="day.day" class="mb-12">
            <h2 class="text-xs font-black text-gray-400 tracking-[0.2em] uppercase mb-6 flex items-center">
              <span class="w-8 h-px bg-gray-300 mr-3"></span> Día {{ day.day }} • {{ day.title }}
            </h2>

            <div class="space-y-6">
              <div v-for="act in day.activities" :key="act.title" class="bg-white p-6 rounded-[2.5rem] shadow-sm border border-gray-100 relative overflow-hidden group">
                <div v-if="act.is_partner" class="absolute top-0 right-0 bg-yellow-400 text-[10px] font-black px-4 py-1.5 rounded-bl-2xl uppercase tracking-tighter">⭐ Favorito del Hotel</div>
                <span class="text-xs font-black text-blue-600 block mb-2">{{ act.time }}</span>
                <h3 class="font-black text-xl mb-3 leading-tight">{{ act.title }}</h3>
                <p class="text-gray-500 text-sm leading-relaxed mb-6">{{ act.description }}</p>
                <div class="flex justify-between items-center border-t border-gray-50 pt-5">
                  <span class="text-[10px] font-bold text-gray-300 uppercase tracking-widest">{{ act.distance_from_hotel_km || "Cerca" }} KM DE TI</span>
                  <button @click="openMap(act.coordinates)" class="text-xs font-black underline underline-offset-4 hover:text-blue-600 transition-colors">VER MAPA</button>
                </div>
              </div>
            </div>
          </div>
